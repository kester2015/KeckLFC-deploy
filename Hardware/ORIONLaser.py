...
class ORIONLaser(Device):
    ...
    def printStatus(self):
        ...
        message = message + str("Product ID: " + ID + ", Ver " + ver + f", SN: {SN}").center(80, '-') + "\n"
        time.sleep(0.1)
    ...
    def readThermisTemp(self):
        Thermistor_V = int.from_bytes(self.query(0x11)['data'], byteorder='big') * 2.5 / 65520
    ...
    def readPhotomonitorV(self):
        return int.from_bytes(self.query(0x13)['data'],
                              byteorder='big') * 2.5 / 65520  # Volts or mA (monitor has 1KOhm resist)

    def readLaserdiodeCur_mA(self, volatile=False):  # return in mA
        if volatile:
            return int.from_bytes(self.query(0x1D)['data'], byteorder='big') / 10
        else:
            return int.from_bytes(self.query(0x26)['data'], byteorder='big') / 10
    ...
    def readTECsetpoint(self, volatile=False):
        if volatile:
            return self.__steinhart_hart_ohm2degC(int.from_bytes(self.query(0x1F)['data'], byteorder='big'))
        else:
            return self.__steinhart_hart_ohm2degC(int.from_bytes(self.query(0x28)['data'], byteorder='big'))
    ...
    def __steinhart_hart_degC2ohm(self, Tc):
        ...
        return int(np.round(np.exp(np.power(y - x / 2, 1 / 3) - np.power(y + x / 2, 1 / 3))))
