...
class AndoOSA_AQ6315E(Device):
    ...
    def __init__(self, addr='GPIB0::1::INSTR', name="ANDO AQ-6315E Optical Spectrum Analyzer"):
        super().__init__(addr=addr, name=name)
        self.__available_traces = ['A', 'B', 'C']
    ...
    def get_reflevel(self):
        return float(self.query('REFL?'))

    def set_reflevel(self, reflevel):
        reflevel = float(reflevel)
        self.query(f'REFL{reflevel:.1f}')
    ...
    def set_resolution(self, res):
        res = float(res)
        self.query(f'RESLN{res:.2f}')
        self.info(self.devicename + f": Resolution set to {res:.2f} nm.")

    def Run(self):
        self.query('RPT')  # equivlant to repeat softkey
        self.info(self.devicename + ": Spectrum collection RUN (repeat) start.")

    def Single(self):
        self.query('SGL')  # equivlant to repeat softkey
        self.info(self.devicename + ": Spectrum collection SINGLE run start.")

    def Stop(self):
        self.query('STP')  # equivlant to repeat softkey
        self.info(self.devicename + ": Spectrum collection STOPped.")

    def set_sens(self, sens):
        sens = str(sens).casefold()
        if sens not in [str(t).casefold() for t in self.__available_sensitivity]:
            raise ValueError(self.devicename + ": Sensitivity " + sens + " unrecognized. " +
                             "AQ6315 Sensitivity must select from " + str(self.__available_sensitivity))
        if sens == 'norm':
            self.query('SNAT')
        elif sens == 'high1':
            self.query('SHI1')
            ...
            self.query('SHI3')
        self.info(self.devicename + ": OSA sensitivity set to " + sens + ".")
    ...
    def get_trace(self, trace, plot=True, filename=None):
        ...
        wl = self.query('WDAT' + trace).strip().split(',')[1:]
