...
class RigolMSO5074(Device):

    def __init__(self, addr="USB0::0x1AB1::0x0515::MS5A255207375::INSTR", name="RigolMSO5074", isVISA=True):
    ...
    def get_trace(self, trace=2, plot=True):
        ...
        #rig.write(':WAV:SOUR CHAN2')
        self.write(':WAV:MODE NORM')
        self.write(':WAV:FORM ASCII')
        self.write(':WAV:POIN 20000')
    ...
    def get_autocorr_FWHM_single(self, data_x=None, data_y=None, filename="autocorr\\test", trace=2, fit_range=50, additional_plotdata=None):
        ...
        raw_FWHM = dx_init*ratio*2*0.648 # raw FWHM directly read from trace
        ...
        plt.axhline(y=thre, color='r', linestyle='-', label = 'Half max')
        plt.legend()
        plt.xlabel("optical time (ps)")
        plt.ylabel("Autocorr (Volt)")
        plt.title(f"zoom in, raw FWHM={raw_FWHM*1e12:.3f} ps")
        ...
        return comb_FWHM*1e12, lorentian(data_x,*popt), fig, raw_FWHM*1e12
