...
class SRS_PIDcontrol_SIM960(object):
    ...
    def printStatus(self):
        ...
        message = message + "|"+ "Output mode: "+self.output_mode+"\n"
        message = message + "|"+ f"Instant Output voltage: {self.output_voltage}\n"
        message = message + "|"+ f"Output limit:{self.output_lowerlim:.3f}V TO {self.output_upperlim:.3f}V\n"
        message = message + "|\t"+ " P,I,D and Offset Status"+"\n"
        message = message + "|\t"+f" P = {self.prop_gain}, action = "+highlight_status(self.prop_action)+"\n"
        message = message + "|\t"+f" I = {self.intg_gain}, action = "+highlight_status(self.prop_action)+"\n"
        message = message + "|\t"+f" D = {self.derv_gain}, action = "+highlight_status(self.prop_action)+"\n"
        message = message + "|\t"+f"Off = {self.outoffset}, action = "+highlight_status(self.prop_action)+"\n"
    ...
    @property
    def setpoint_input(self): # will be 'EXT' or 'INT'
        return self.get_setpoint_input()
    @setpoint_input.setter
    def setpoint_input(self,status): # available_status = ['0','1','int','ext']
        self.set_setpoint_input(status)
    @property
    def setpoint(self):
    ...
    def set_prop_gain(self, num):
        cmd = "GAIN"
        num = num is str(num).isnumeric() else self.__decode_str_to_SIunit(num)
        from math import log10
        self.__set_num_withcmd(cmd,num,printstr="Proportional Gain in V/V", 
                               low_lim=-self.__prop_gain_max, high_lim=self.__prop_gain_max, 
    ...
    def set_intg_gain(self, num):
        ...
        num = num is str(num).isnumeric() else self.__decode_str_to_SIunit(num)
        from math import log10
        self.__set_num_withcmd(cmd,num,printstr="Integral Gain in 1/seconds", 
                               low_lim=self.__intg_gain_min, high_lim=self.__intg_gain_max, 
    ...
    def set_derv_gain(self, num):
        ...
        num = num is str(num).isnumeric() else self.__decode_str_to_SIunit(num)
        from math import log10
        self.__set_num_withcmd(cmd,num,printstr="Derivative Gain in seconds", 
                               low_lim=self.__derv_gain_min, high_lim=self.__derv_gain_max, 
    ...
    def set_outoffset(self, num):
        ...
        num = num is str(num).isnumeric() else self.__decode_str_to_SIunit(num)
        from math import log10
        self.__set_num_withcmd(cmd,num,printstr="Output offset in Volts", 
                               low_lim=self.__outoffset_min, high_lim=self.__outoffset_max, 
    ...
    def set_output_mode(self, status):
        ...
        if status not in available_status:
            raise ValueError(self.devicename+": Unrecognized output mode "+status+
    ...
    def get_setpoint_input(self):
        ...
        if r=='0':
            ...
            return 'EXT'
    def set_setpoint_input(self, status):
        status=str(status).casefold()
    ...
    def set_setpoint(self, num):
        ...
        num = num is str(num).isnumeric() else self.__decode_str_to_SIunit(num)
        from math import log10
        self.__set_num_withcmd(cmd,num,printstr="Locking set point in Volt", 
                               low_lim=self.__setpoint_min, high_lim=self.__setpoint_max, 
    ...
    def set_spramprate(self, num):
        cmd = "RATE"
        num = num is str(num).isnumeric() else self.__decode_str_to_SIunit(num)
        self.__set_num_withcmd(cmd,num,printstr="Setpoint Ramping Rate in V/s", 
                               low_lim=self.__spramprate_min, high_lim=self.__spramprate_max) 
    ...
    def set_manual_output(self, num):
        ...
        num = num is str(num).isnumeric() else self.__decode_str_to_SIunit(num)
        from math import log10
        self.__set_num_withcmd(cmd,num,printstr="Output in Manual Mode in Volt", 
                               low_lim=self.__manual_output_min, high_lim=self.__manual_output_max, 
    ...
    def __set_onoff_withcmd(self, cmd, status, printstr=None):
        ...
        if status in set(['1', 'on']):
            ...
            raise ValueError(self.devicename+": Unrecognized ON/OFF string ["+status+"] given. Should be 0|1|'on'|'off' ")
    ...
    def __set_num_withcmd(self,cmd,num,printstr=None,low_lim=None,high_lim=None,decimal=None):
        ...
        if not decimal==None: # will be deprecated by resolution
            ...
            if not num*10**decimal % 1==0:
                warnings.warn(self.devicename+": Setting "+printstr+" precision overgiven, "
                              +str(decimal)+" decimal degits required. Given number "+str(num)+" is rounded.")
