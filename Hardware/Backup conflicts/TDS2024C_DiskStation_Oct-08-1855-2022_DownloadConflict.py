...
class TDS2024C(Device):
    ...
    def get_autocorr_FWHM_single(self, data_x=None, data_y=None, filename="autocorr\\test", trace=2, fit_range=50, additional_plotdata=None):
        ...
        if data_x==None or data_y==None:
            self.save_trace(filename=filename, trace=trace, extensions=['.mat'], plot=False)
        ...
        dx_init = data_x[pos2]-data_x[pos1]
        ...
        plt.title(f"zoom in")
        plt.show()
        plt.savefig(filename+f"_{comb_FWHM*1e12:.3f}ps.png")

        return comb_FWHM*1e12
    ...
    def save_trace(self, filename, trace=2, extensions=['.mat', '.txt'], plot=True):
