...
class AmonicsEDFA(Device):
    ...
    def printStatus(self):
        ...
        message = message + "|\t\t Set " + ("Cur" if m == 'ACC' else "Pwr") + ": " + str(
            self.accCh1Cur) + (" mA" if m == 'ACC' else " mW") + "\n"
        message = message + "|\t\t Status: " + highlight_status(self.accCh1Status) + "\n"
        message = message + "|\t\t Input Power: " + str(self.inputPowerCh1) + " mW\n"
    ...
    @accCh2Cur.setter
    def accCh2Cur(self, cur1):  # cur1 is numeric or str like '0.3A', '250mA'
        cur1 = str(cur1)
        if not cur1.isnumeric():
            cur1 = self.__current_str_to_mA(cur1)
        else:
            cur1 = float(cur1)
        self._setIorP(cur1, channel=2, mode=self.modeCh1)

    @property
    def accCh2Status(self):  # value will be 'OFF'|'ON'|'BUSY'|'LOCK'
        return self._getChStatus(channel=2, mode=self.modeCh1)
